...
### script edited 24042023 to work with variants that were merge with clinvar using the aminoacid change
...
d = fread(paste0(run_path, "/results/genotypes/aa_change_merge/", folder, "/", file_name, ".txt"))
d_t = data.frame(t(d))
...
write.table(hets, 
	paste0(run_path, "/results/genotypes/aa_change_merge/", file_name, 
			sprintf("_%s_%s_aa_change_merge_hets.txt", ref_a, alt_a)), 
...
if (nrow(homo) >=1) {
	...
	write.table(homo, 
		paste0(run_path, "/results/genotypes/aa_change_merge/", file_name, 
				sprintf("_%s_%s_aa_change_merge_homo.txt", ref_a, alt_a)), 
		sep = "\t", dec = ".", quote = FALSE, row.names = FALSE)
}
